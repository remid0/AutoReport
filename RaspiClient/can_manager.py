...
class CanManager(object):
    def __init__(self, vehicule_channel, pc_channel, bus_type): #PC channel or MABX channel ?
        ...
        self.vehicule_sate = Value(c_uint)
        ...
        receiver_process = Process(target=CanReceiver.receive, args=(self.odometer_value, self.odometer_time, self.vin, self.vehicule_state, vehicule_bus))
        receiver_process.start()

    def get_odometer_info(self):
        return OdometerInfo(self.odometer_value.value, self.odometer_time.value)

    def get_vehicule_state(selfself):
        pass
        #possible states :
